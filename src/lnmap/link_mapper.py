...
class Link:
    ...
    @property
    def key(self) -> int | Path:
        """The value identifying this link group: the shared inode for hard
        links, or the shared target path for symlinks and aliases. Used where
        display code wants one value regardless of link_type."""
        return self.inode if self.link_type == "hard" else self.target
    ...
    def consolidate_links(self, hard_rows, sym_rows, alias_rows) -> list[Link]:
        ...
        for inode, paths in hard_map.items():
            if len(paths) > 1:
                ...
                links.append(Link(link_type="hard", inode=inode, paths=tuple(paths)))
        ...
        for target, paths in sym_map.items():
            if paths:
                links.append(Link(link_type="sym", target=target, paths=tuple(paths)))
        ...
        for target, paths in alias_map.items():
            if paths:
                links.append(Link(link_type="alias", target=target, paths=tuple(paths)))
...
def _macos_protected_dirs() -> set[Path]:
    """Absolute paths of TCC-protected directories under the user's home, as
    configured in the lnmap config file (see lnmap.config)."""
    if sys.platform != "darwin":
        return set()
    home = Path.home().resolve()
    return {home / sub for sub in protected_subpaths()}
